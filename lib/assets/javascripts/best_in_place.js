...
BestInPlaceEditor.prototype = {
  ...
  loadErrorCallback : function(request, error) {
    ...
    this.element.trigger(jQuery.Event("ajax:error"), request.responseText);
...
BestInPlaceEditor.forms = {
  "input" : {
    ...
    inputBlurHandler : function(event) {
      // If I have an OK and Cancel button, why would I want blur events to trigger?
      if (event.data.editor.okButton && event.data.eidtor.cancelButton) {
        return false;
      }
  ...
  "textarea" : {
    ...
    // When buttons are present, use a timer on the blur event to give precedence to clicks
    blurHandler : function(event) {
      // Don't blur if there are buttons
...
BestInPlaceEditor.helpers = {
  ...
  okButtonRenderer : function(element) {
    if (!element.okButton) return '';

    input_class = '';
    if (element.okButtonClass) {
      input_class = 'class="' + element.okButtonClass + '"';
    }
    return '<input type="submit" value="' + element.okButton + '" ' + input_class + ' />';
  },
  ...
  cancelButtonRenderer : function(element) {
    ...
    return '<input type="button" value="' + element.cancelButton + '" ' + input_class + ' />';
  }
}
